...
from unittest.mock import create_autospec, MagicMock, call, patch, Mock
...
@patch("exasol_script_languages_container_ci.lib.release_uploader.TemporaryDirectory",
       autospec=True)
def test(temp_dir_mock):
    asset_uploader_mock: Union[MagicMock, AssetUploader] = create_autospec(AssetUploader)
    ...
    assert \
        mock_cast(asset_uploader_mock.upload_assets).mock_calls == [
            call(
        ...
        and mock_cast(ci_export_mock.export).mock_calls == [
            call(flavor_path=('test_flavor_path',),
                 export_path=expected_artifact_path)
        ]
