...
class TestSuccessfulFlavor(BaseCIExecuteTest):
    ...
    def complete_setup(self, base_setup):
        ...
        self.linker_namespace_tests_all_tests_result = (
            self.create_all_tests_result_mock(tests_are_ok=True)
        )
    ...
    def test_ci_step_output_printer_call(self, complete_setup):
        ...
        assert self.printer_mock.mock_calls == [
            ...
            call.print_file(
                self.linker_namespace_tests_all_tests_result.command_line_output_path
            ),
...
class TestFailingRunDBTestFlavor(BaseCIExecuteTest):
    ...
    @pytest.fixture(params=[(False, True), (True, False), (False, False)])
    def complete_setup(self, base_setup, request):
        self.db_tests_all_tests_result = self.create_all_tests_result_mock(
            tests_are_ok=request.param[0]
        )
        self.linker_namespace_tests_all_tests_result = (
            self.create_all_tests_result_mock(tests_are_ok=request.param[1])
        )
        mock_cast(self.db_test_runner_mock.run).side_effect = [
            self.db_tests_all_tests_result,
            self.linker_namespace_tests_all_tests_result,
